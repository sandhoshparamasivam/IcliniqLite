...
//      holder.tv_speciality.setTypeface(robo_reg);
        ...
        if (holder.imageview_poster != null && null != objBean.getArtimgurl() && objBean.getArtimgurl().trim().length() > 0) {
            ...
            Picasso.with(getContext()).load(objBean.getArtimgurl()).placeholder(R.mipmap.place_holder).error(R.mipmap.doctor_icon).into(holder.imageview_poster);
        } else {
...
*/
            Picasso.with(getContext()).load(objBean.getCdurl()).placeholder(R.mipmap.place_holder).error(R.mipmap.tips_banner).into(holder.img_qasebanner);
